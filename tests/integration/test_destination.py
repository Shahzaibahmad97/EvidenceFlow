...
def test_a_record_is_created_once(session):
    crm = DatabaseCrm(session)
...
def test_the_destination_refuses_a_second_record_for_the_same_invoice(session):
    crm = DatabaseCrm(session)
...
def test_a_different_invoice_from_the_same_supplier_is_accepted(session):
    crm = DatabaseCrm(session)
    crm.create_record("key-one", PAYLOAD)
...
def test_the_same_number_from_a_different_supplier_is_accepted(session):
    crm = DatabaseCrm(session)
    crm.create_record("key-one", PAYLOAD)
...
def test_supplier_punctuation_does_not_create_a_second_record(session):
    crm = DatabaseCrm(session)
    crm.create_record("key-one", PAYLOAD)
...
def test_a_payload_without_a_business_key_is_rejected(session):
    crm = DatabaseCrm(session)
...
def test_the_destination_is_shared_across_clients(session):
    first = DatabaseCrm(session)
    second = DatabaseCrm(session)
...
def test_a_record_written_by_one_process_is_visible_to_another(file_session_factory):
    from app.repositories.session import session_scope

    with session_scope(file_session_factory) as writer:
        DatabaseCrm(writer).create_record("key-one", PAYLOAD)

    with session_scope(file_session_factory) as reader:
        assert not DatabaseCrm(reader).create_record("key-one", PAYLOAD).created
        assert reader.query(CrmRecordRow).count() == 1
