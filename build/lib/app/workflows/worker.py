...
class Worker:
    ...
    def run_forever(self, stop: threading.Event, idle_sleep: float = IDLE_SLEEP_SECONDS) -> None:
        """Survives an unexpected failure: a dead worker thread is an invisible outage."""
        while not stop.is_set():
            try:
                idle = self.run_once() is None
            except Exception:
                logger.exception("worker %s failed outside a job", self.worker_id)
                stop.wait(ERROR_SLEEP_SECONDS)
                continue
            if idle:
